...
class MinibatchRlEvalWandb(MinibatchRlEval):
    ...
    def startup(self):
        ...
        self.algo.gb_collector = self.sampler.collector
        ...
        for itr in range(n_itr):
            ...
            with logger.prefix(f"itr #{itr} "):
                ...
                if (itr + 1) % self.log_interval_itrs == 0:
                    ...
                    mean_score = np.mean([e["GameScore"] for e in eval_traj_infos])
                    self.record["mean_episode_return"].append(mean_score)
                    self.record["step"].append(itr+1)
                    self.record["number_of_states"].append(self.sampler.collector.s2i.max)
                    self.log_diagnostics(itr, eval_traj_infos, eval_time)
                    pd.DataFrame(self.record).to_csv(f"{self.log_path}/logs.csv")
        self.shutdown()
...
class SerialSampler(BaseSampler):
    ...
    def __init__(self, *args, CollectorCls=CpuResetGraphCollector,
