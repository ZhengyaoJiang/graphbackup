...
def integrate_table(tasks, indexes, labels, dir, steps, name, repeats, summary, human_scores, format="text"):
        ...
        for task_n, task in enumerate(tasks):
            ...
            for round in range(repeats):
                ...
                try:
                    ...
                    if human_scores:
                        last = df["mean_episode_return"].iloc[-10:]# / human_scores[task_n] * 100
                        random_scores = returns.iloc[5]
                        returns = (last-random_scores) / (float(human_scores[task_n])-random_scores) *100
                    else:
                        last = df["mean_episode_return"].iloc[-10:]# / human_scores[task_n] * 100
        ...
        for task_n, task in enumerate(tasks):
            ...
            for round in range(repeats):
                ...
                try:
                    ...
                    if human_scores:
                        returns = df["mean_episode_return"].ewm(span=10).mean()
...
def main(flags):
    ...
    if flags.mode == "integrate_table":
        integrate_table(flags.tasks, flags.idx, flags.labels, flags.dir, flags.steps, flags.name,
                        repeats=flags.repeats, human_scores=flags.human_scores, summary=flags.summary)
