...
def integrate_table(tasks, indexes, labels, dir, steps, name, repeats, summary, human_scores, random_scores,
                    format="text"):
            ...
            for round in range(repeats):
                ...
                try:
                    ...
                    if human_scores:
                        ...
                        random_scores = float(random_scores[task_n])
                        last = (last-random_scores) / (float(human_scores[task_n])-random_scores) *100
                    else:
                        last = df["mean_episode_return"].iloc[-10:]# / human_scores[task_n] * 100
...
def main(flags):
    ...
    if flags.mode == "integrate_table":
        integrate_table(flags.tasks, flags.idx, flags.labels, flags.dir, flags.steps, flags.name,
                        ...
                        random_scores=flags.random_scores, summary=flags.summary)
